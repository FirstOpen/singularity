...
public class SGLN extends EPC {
	protected String sManagerId;
	protected String sLocationReference;
	...
	public SGLN(BitSet inEPC, Properties iConfig) throws EPCParseException {
		super(inEPC, iConfig);
		if(inEPC.length() == 64) {
	 ...
	 */
	public SGLN(String inEPC, int flags, Properties iConfig) throws EPCParseException {
	...
	public SGLN(byte[] inEPC,Properties config) throws EPCParseException {
		super(inEPC,config);
		switch((inEPC[0] & 0xFF)) {
		case 9: parse64bit(inEPC);
		break;
		case 50: parse96bit(inEPC);
		break;
		default:
			throw new EPCParseException("Binary EPC's header does not correspond to an SGLN");
		}
	}
	...
	private void parse64bit(byte[] inEPC) throws EPCParseException {
		...
		sLocationReference=utils.ZeroPad(Long.toString(ParseUtils.BitsToInt(bits.get(26,46),20)),(12-sManagerId.length()));
	...
	private void parse96bit(byte[] inEPC) throws EPCParseException {
		...
		sSerialNum = Long.toString(ParseUtils.BitsToInt(bits.get(56,97),41));
		...
		switch((int) lPartition) {
			case 0:
				...
				sLocationReference =
				   Long.toString(ParseUtils.BitsToInt(bits.get(55,56),1));
				break;
			case 1:
				sManagerId = utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(15,52),37)),11);
				sLocationReference =
				   Long.toString(ParseUtils.BitsToInt(bits.get(52,56),4));
				break;
			case 2:
				sManagerId =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(15,49),34)),10);
				sLocationReference =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(49,56),7)),2);
				break;
			case 3:
				sManagerId =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(15,45),30)),9);
				sLocationReference =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(45,56),11)),3);
				break;
			case 4:
				sManagerId =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(15,42),27)),8);
				sLocationReference =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(42,56),14)),4);
				break;
			case 5:
				sManagerId =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(15,39),24)),7);
				sLocationReference =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(39,56),17)),5);
				break;
			case 6:
				sManagerId =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(15,35),20)),6);
				sLocationReference =utils.ZeroPad(
				   Long.toString(ParseUtils.BitsToInt(bits.get(35,56),21)),6);
				break;
		       default: 
	...
	private void parseCanonical(String inEPC) throws EPCParseException {
		...
		if(!sIdType.equalsIgnoreCase("sgln")) {
			throw new EPCParseException("Not an SGLN. Type is " + sIdType);
		}
		sManagerId = st.nextToken();
		if(sManagerId == null) {
			throw new EPCParseException("No manager id found");
		}
		sLocationReference = st.nextToken();
		if(sLocationReference == null) {
			throw new EPCParseException("No location reference found");
		}
	...
	public String getCanonicalEncoding() {
		return("urn:epc:id:sgln:" + sManagerId + "." + sLocationReference + "." + sSerialNum);
	}
	public String getONSEncoding() {
		return(sLocationReference + "." + sManagerId + ".sgln.id.");
	}
	...
	private BitSet build64bit() throws EPCParseException {
		...
		String bsLocationReference = utils.ZeroPad(Integer.toString(Integer.parseInt(sLocationReference),2),20);
		for(int i=0;i<bsLocationReference.length();i++) 
			if(bsLocationReference.charAt(i) == '1') 
				mybits.set(i+26);
			else 
				mybits.set(i+26,false);
		String bsSerialNum = utils.ZeroPad(Integer.toString(Integer.parseInt(sSerialNum),2),19);
		for(int i=0;i<bsSerialNum.length();i++) 
			if(bsSerialNum.charAt(i) == '1') 
				mybits.set(i+46);
			else 
				mybits.set(i+46,false);
		return(mybits);
	}
	...
	private BitSet build96bit() throws EPCParseException {
		...
		case 6:
			...
			for(int i=0;i<bsManagerId.length();i++) {
			 ...
			 bsLocationReference = utils.ZeroPad(Integer.toString(Integer.parseInt(sLocationReference),2),21);
			for(int i=0;i<bsLocationReference.length();i++) {
				if(bsLocationReference.charAt(i) == '1') 
					mybits.set(35+i);
				else 
			 ...
			 bsSerialNum = utils.ZeroPad(Integer.toString(Integer.parseInt(sSerialNum),2),41);
			for(int i=0;i<bsSerialNum.length();i++) {
				if(bsSerialNum.charAt(i) == '1') 
					mybits.set(56+i);
				else
					mybits.set(56+i,false);
			}
			break;
		case 7:
			...
			for(int i=0;i<bsManagerId.length();i++) {
			 ...
			 bsLocationReference = utils.ZeroPad(Integer.toString(Integer.parseInt(sLocationReference),2),17);
			for(int i=0;i<bsLocationReference.length();i++) {
				if(bsLocationReference.charAt(i) == '1') 
					mybits.set(39+i);
				else 
		...
		case 8:
			...
			for(int i=0;i<bsManagerId.length();i++) {
			 ...
			 bsLocationReference = utils.ZeroPad(Integer.toString(Integer.parseInt(sLocationReference),2),14);
			for(int i=0;i<bsLocationReference.length();i++) {
				if(bsLocationReference.charAt(i) == '1') 
					mybits.set(42+i);
				else 
		...
		case 9:
			...
			for(int i=0;i<bsManagerId.length();i++) {
			 ...
			 bsLocationReference = utils.ZeroPad(Integer.toString(Integer.parseInt(sLocationReference),2),11);
			for(int i=0;i<bsLocationReference.length();i++) {
				if(bsLocationReference.charAt(i) == '1') 
					mybits.set(45+i);
				else 
		...
		case 10:
			...
			for(int i=0;i<bsManagerId.length();i++) {
			 ...
			 bsLocationReference = utils.ZeroPad(Integer.toString(Integer.parseInt(sLocationReference),2),7);
			for(int i=0;i<bsLocationReference.length();i++) {
				if(bsLocationReference.charAt(i) == '1') 
					mybits.set(49+i);
				else 
		...
		for(int i=0;i<bsManagerId.length();i++) {
		 ...
		 bsLocationReference = utils.ZeroPad(Integer.toString(Integer.parseInt(sLocationReference),2),4);
		for(int i=0;i<bsLocationReference.length();i++) {
			if(bsLocationReference.charAt(i) == '1') 
				mybits.set(56+i);
			else 
		...
		case 12:
			...
			for(int i=0;i<bsManagerId.length();i++) {
			 ...
			 bsLocationReference = utils.ZeroPad(Integer.toString(Integer.parseInt(sLocationReference),2),1);
			for(int i=0;i<bsLocationReference.length();i++) {
				if(bsLocationReference.charAt(i) == '1') 
					mybits.set(56+i);
				else 
					mybits.set(26+i,false);
			}
			 bsSerialNum = utils.ZeroPad(Integer.toString(Integer.parseInt(sSerialNum),2),41);
			for(int i=0;i<bsSerialNum.length();i++) {
				if(bsSerialNum.charAt(i) == '1') 
	 ...
	 */
	public String getlocationRef(int flags) {
		if((flags & EPC.BINARY) != 0) {
			return (Integer.toBinaryString(Integer.parseInt(sLocationReference)));
		} else if((flags & EPC.HEX) != 0) {
			return (Integer.toHexString(Integer.parseInt(sLocationReference)));			
		} else if((flags & EPC.DECIMAL) != 0) {
			return(sLocationReference);
		}
		return(null);
	}
	...
	/**
	 * @param locationRef The slocationRef to set.
	 */
	public void setLocationReference(String locationRef,int flags) {
		if((flags & EPC.BINARY) != 0) {
			sLocationReference  = Integer.toString(Integer.parseInt(locationRef,2));
		} else if((flags & EPC.HEX) != 0) {
			sLocationReference  = Integer.toString(Integer.parseInt(locationRef,16));
		} else if((flags & EPC.DECIMAL) != 0) {
			sLocationReference = locationRef;
		}
	}
