...
<f:subview id="ecspec-list-view">

    <h:panelGrid columns="1" styleClass="sm-table-title">
        <f:facet name="header">
            <h:outputText value="Define ECSpec List" styleClass="sm-table-title" />
        </f:facet>
        <h:panelGroup>
            <h:dataTable id="ecspec-list-datatable" rows="50" styleClass="sm-table-background" rowClasses="sm-table-odd-row,sm-table-even-row" cellpadding="3" value="#{ecSpecBean.list}" var="crow" binding="#{ecSpecBean.uiTable}">
                <h:column>
                    <f:facet name="header">
                        <h:outputText value="Name" styleClass="sm-table-header" />
                    </f:facet>
                    <h:commandLink styleClass="sm-table-value" actionListener="#{ecSpecBean.getDetail}">
                        <h:graphicImage url="/images/create.gif" alt="ECSpec Name" styleClass="sm-table-toolbar-icon" />
                        <h:outputText value="#{crow.specName}" styleClass="sm-table-value" />
                    </h:commandLink>
                </h:column>
                <h:column>
                    <f:facet name="header">
                        <h:outputText value="Current State" styleClass="sm-table-header" />
                    </f:facet>
                    <h:outputText value="#{crow.currentState}" styleClass="sm-table-value" />
                </h:column>
            </h:dataTable>
              <h:panelGrid columns="1" styleClass="scrollerTable2" columnClasses="standardTable_ColumnCentered" >
            <x:dataScroller id="scroll_1"
                    for="ecspec-list-datatable"
                    fastStep="1"
